...
export class CollaborationComponent implements OnInit {
  ...
  onDelete(i) {
    this.service
      .deleteDialog("Are you sure you want to delete this user permanently?")
      .subscribe(
        (res) => {
  ...
  addNewUserDialog() {
    this.service.addNewUserDialog().subscribe(
      (res) => {
  ...
  getAddUserPopupResponse(response: any) {
    if (response) {
      ...
      this.service.addNewCollabration(addUser).subscribe((res) => {
